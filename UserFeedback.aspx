<%@ Page Title="" Language="C#" MasterPageFile="~/master_User.master" AutoEventWireup="true" CodeFile="UserFeedback.aspx.cs" Inherits="UserFeedback" %>
...
<div style="margin-left:20%;">
    ...
    <asp:GridView ID="gvItemRequest" runat="server" AutoGenerateColumns="false" AllowPaging="true">
        <Columns>
            ...
            <asp:TemplateField>
                <ItemTemplate>
                    <asp:LinkButton Text="Add Feedback" runat="server" CssClass="AddFeedback" />
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
    ...
    <script type="text/javascript">
        $(function () {
            $.ajax({
                type: "POST",
                url: "UserFeedback.aspx/GetItemRequestStatus",
                data: '{}',
        ...
        function AppendRow(row, TransId, DateRequest, NoOfPerson, DateEvent, PurposeOfMeeing, DebitCode, Venue,
                            Time, DeptName, PreparedBy, Status) {
        ...
        $("body").on("click", "[id*=gvItemRequest] .ShowDetails", function () {
            ...
            window.open("ShowDetail.aspx?Text=" + TransId, "_blank", "WIDTH=980,HEIGHT=590,scrollbars=no, menubar=no,resizable=yes,directories=no,location=no");

        });

        
        $("body").on("click", "[id*=gvItemRequest] .AddFeedback", function (event) {
            var row = $(this).closest("tr");
            var TransId = $(".Trans_Id", row).find("span").html();

            event.preventDefault();
            window.location.href = "../COS/AddFeedback.aspx?Text=" + TransId;
