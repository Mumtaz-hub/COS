<%@ Page Title="" Language="C#" MasterPageFile="~/master_User.master" AutoEventWireup="true" CodeFile="ShowOrder.aspx.cs" Inherits="Status" %>
...
<div style="margin-left:10%;">
    ...
    <asp:GridView ID="gvItemRequest" runat="server" AutoGenerateColumns="false" AllowPaging="true">
        <Columns>
            ...
            <asp:TemplateField HeaderText="Status" ItemStyle-Width="150px" ItemStyle-CssClass="Status" Visible="false">
                <ItemTemplate>
            ...
            <asp:TemplateField HeaderText="Status I&S Dept." ItemStyle-Width="150px" ItemStyle-CssClass="Status_ISDept" Visible="false">
                <ItemTemplate>
                    <asp:Label  Text='<%# Eval("Status_ISDept") %>' runat="server" />
                </ItemTemplate>
            </asp:TemplateField
            >
            <asp:TemplateField HeaderText="User EmailID" ItemStyle-Width="150px" ItemStyle-CssClass="UserEmailID" Visible="false">
                <ItemTemplate>
                    <asp:Label Text='<%# Eval("UserEmailID") %>' runat="server" />
                </ItemTemplate>
            </asp:TemplateField>

            <asp:TemplateField HeaderText="HOD EmailID" ItemStyle-Width="150px" ItemStyle-CssClass="HODEmailID" Visible="false">
                <ItemTemplate>
                    <asp:Label Text='<%# Eval("HODEmailID") %>' runat="server" />
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="I & S Dept. EmailID" ItemStyle-Width="150px" ItemStyle-CssClass="ISDEPTEmailID" Visible="false">
                <ItemTemplate>
                    <asp:Label Text='<%# Eval("ISDEPTEmailID") %>' runat="server" />
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField>
                <ItemTemplate>
            ...
            <asp:TemplateField>
                <ItemTemplate>
                    <asp:LinkButton Text="Approved" runat="server" CssClass="Approved" />
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
    ...
    <script type="text/javascript">
        $(function () {
            $.ajax({
                type: "POST",
                url: "ShowOrder.aspx/GetItemRequestStatus",
                data: '{}',
        ...
        function OnSuccess(response) {
            ...
            $.each(Details, function () {
                ...
                $(this).find("UserEmailID").text(),
                $(this).find("HODEmailID").text(),
                $(this).find("ISDEPTEmailID").text());
        ...
        function AppendRow(row,TransId,DateRequest, NoOfPerson, DateEvent, PurposeOfMeeing, DebitCode, Venue,
                            Time, DeptName, PreparedBy, Status, Staus_HOD, Status_ISDept, UserEmailID, HODEmailID, ISDEPTEmailID) 
        {
            ...
            $(".UserEmailID", row).find("span").html(UserEmailID);
            $(".HODEmailID", row).find("span").html(HODEmailID);
            $(".ISDEPTEmailID", row).find("span").html(ISDEPTEmailID);

            $("[id*=gvItemRequest]").append(row);
        }

        //Button event handler.
        $("body").on("click", "[id*=gvItemRequest] .Approved", function () {
            ...
            $.ajax({
                type: "POST",
                url: "ShowOrder.aspx/UpdateStatus",
